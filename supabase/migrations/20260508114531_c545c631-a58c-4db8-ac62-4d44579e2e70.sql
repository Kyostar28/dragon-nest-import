...
-- Trigger: on new auth user, create profile + slots 1,2 + 1 nest + 1 random common egg
...
BEGIN
  INSERT INTO public.profiles (id, wallet_address)
    VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'wallet_address', NEW.email));
